# Euro Bills - Setup help

Everything you need to get **Euro Bills** running, step by step. If something goes wrong, check *Troubleshooting* at the end.
...
1. Download **`Euro-Bills-v1.1.1-alpha.mcaddon`** from the [Releases page](../../releases) - or directly from this repository.
...
- Summon a bank NPC: `/summon custom:bank_teller` (players use it to withdraw, deposit and pay) and `/summon custom:bank_admin` (operators: bank settings).
- Set the scoreboard objective that stores balances in the Bank Admin menu (default name is configurable).
- Command blocks: `/scriptevent geld:give <amount>` and `/scriptevent geld:take <amount>`.
